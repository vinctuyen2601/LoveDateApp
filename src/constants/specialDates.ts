 ...
 */
export function resolveSpecialDateForYear(sd: SpecialDate, year: number): Date | null {
  if (sd.month && sd.day) {
    return new Date(year, sd.month - 1, sd.day);
  }
  if (sd.nthWeekday) {
    return getNthWeekday(year, sd.nthWeekday.month, sd.nthWeekday.weekday, sd.nthWeekday.nth);
  }
