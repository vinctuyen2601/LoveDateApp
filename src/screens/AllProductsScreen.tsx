...
const AllProductsScreen: React.FC = () => {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      setAiReasoning('');
      setAiResults([]);
      const data = await apiService.post('/products/ai-suggest', { prompt: aiPrompt.trim() });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.modeToggle}>
        ...
        >
          <Ionicons name="grid-outline" size={14} color={!aiMode ? COLORS.white : COLORS.textSecondary} />
          <Text style={[styles.modeBtnText, !aiMode && styles.modeBtnTextActive]}>Duyệt sản phẩm</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.modeBtn, aiMode && styles.modeBtnAiActive]}
          onPress={() => setAiMode(true)}
        >
          <Ionicons name="sparkles" size={14} color={aiMode ? COLORS.white : '#D97706'} />
          <Text style={[styles.modeBtnText, aiMode && styles.modeBtnAiTextActive]}>Tìm với AI</Text>
        </TouchableOpacity>
      </View>
        ...
        <>
          ...
          <View style={styles.categoryBar}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
              ...
              >
                <Text style={[styles.chipText, categoryId === 'all' && styles.chipTextActive]}>Tất cả</Text>
              </TouchableOpacity>
              {productCategories.map((cat) => (
                <TouchableOpacity
                  key={cat.id}
                  style={[styles.chip, styles.chipWithIcon, categoryId === cat.id && { backgroundColor: cat.color, borderColor: cat.color }]}
                  onPress={() => setCategoryId(cat.id)}
                >
                  <Ionicons name={cat.icon as any} size={14} color={categoryId === cat.id ? COLORS.white : cat.color} />
          ...
          <View style={styles.filterRow}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
              {BUDGET_OPTIONS.map((f) => (
                ...
                >
                  <Text style={[styles.chipText, budget === f.key && styles.chipTextActive]}>{f.label}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          ...
          {loading ? (
            ...
            <View style={styles.center}>
              ...
              <Text style={styles.stateText}>
                {activeFilterCount > 0 ? 'Không tìm thấy sản phẩm phù hợp' : 'Chưa có sản phẩm nào'}
              </Text>
              {activeFilterCount > 0 && (
        ...
        >
          ...
          <View style={styles.aiCard}>
            ...
            <View style={styles.inputWrap}>
              ...
              />
              {aiPrompt.length > 0 && (
                <TouchableOpacity style={styles.clearBtn} onPress={() => setAiPrompt('')}>
                  <Ionicons name="close-circle-outline" size={16} color={COLORS.textSecondary} />
                </TouchableOpacity>
              )}
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.ideasRow}>
            ...
            <TouchableOpacity
              style={[styles.generateBtn, (aiLoading || !aiPrompt.trim()) && styles.generateBtnDisabled]}
          ...
          {aiReasoning ? (
            <View style={styles.reasoningBanner}>
              ...
              <TouchableOpacity onPress={() => { setAiResults([]); setAiReasoning(''); }}>
          ...
          {!aiLoading && aiResults.length > 0 && (
            <>
              <Text style={styles.aiResultCount}>{aiResults.length} gợi ý cho bạn</Text>
              {aiResults.map((p) => (
...
const styles = StyleSheet.create({
  ...
  // AI empty state
  aiEmptyState: { alignItems: 'center', justifyContent: 'center', paddingVertical: 56 },
  aiEmptyIcon: { fontSize: 52, marginBottom: 16 },
  aiEmptyTitle: { fontSize: 18, fontWeight: '700', color: COLORS.textPrimary, marginBottom: 8 },
  aiEmptyText: { fontSize: 14, color: COLORS.textSecondary, textAlign: 'center', paddingHorizontal: 28, lineHeight: 21 },
