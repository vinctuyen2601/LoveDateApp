...
export class NotificationUtils {
  ...
  static async scheduleNotification(
    ...
    try {
      ...
      // For one-time events: skip if notification date is in the past.
      // For recurring events: always schedule (CALENDAR trigger fires next year automatically).
      if (!event.isRecurring && notificationDate < new Date()) {
        return null;
      }
